        ...
        if (isSuccess)
        {
            ...
            while (true)
            {
                ...
                else if (requestPath == "/login")
                {
                    ...
                    var login = WebUtility.UrlDecode(buffer[0][6..]);
                    Console.WriteLine(login);
                    var pass = WebUtility.UrlDecode(buffer[1]);
                    var isSend = await MailSender.SendEmailAsync(login, pass, "Hello from Liza","//Users//lizaka//Desktop//battle.zip");
                    if(isSend)
